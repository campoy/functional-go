...
The alternatives were worse. Declaring pointer receivers instead would contradict slide 57 and break the method expressions slide 60 is specifically about; dereferencing inside `Maybe.Map` would contradict slide 78, which stores `r` — the pointer — unchanged. So `argValue` in `fp/func.go` dereferences a pointer whose element type is assignable to the parameter type, and `Compose`'s and `Do`'s type checks allow for it. A nil pointer has nothing to dereference and panics; see the entry below.
...
**Slide 78's nil-pointer check is applied at both ends of `Maybe.Map`, not just one.** Slide 78 checks the value coming *out* of `f`; the guard on the way *in* is slide 52's `m.Value == nil`, which a typed nil pointer passes, because a nil `*T` in an `interface{}` is not a nil interface. That asymmetry had a consequence: `Map` called the step anyway, `argValue` found a nil `*T` where a `T` was wanted and substituted the zero value, and the step's answer came back as an ordinary success. `Maybe{(*box)(nil)}.Do(box.Get)` returned `(Maybe{0}, nil)` — a fabricated number, indistinguishable from a real one, with no error.

Nothing about a typed nil the caller supplies is different from one a step produced, and the shape is not hypothetical: slide 61 is *about* chaining method expressions over pointer-returning methods, so `Maybe{p.Address()}.Do(Address.City, ...)` — starting one link down the talk's own chain — is the natural thing to write and is exactly what broke. The inline check is now `isNilPtr` in `fp/maybe.go` and both ends of `Map` call it. This does not change slide 78's semantics; it applies them in one more place.

The other half is `argValue`. A nil pointer used to become `reflect.Zero(in)`; it now panics with `can't dereference a nil *fp.box to call a function taking fp.box`. `Call` has no error result, so a panic is the only way it can refuse, and refusing loudly beats returning a wrong number quietly. `Maybe` never reaches it now, but `Many` does — it has no notion of an absent element, so a step returning a typed nil mid-chain leaves a nil pointer for the next step, and that chain is now loud rather than wrong. The panic follows how `Each` already reports its own misuse. The doc comment above `argValue` claimed `Maybe.Map` short-circuited "before that can happen, so it only matters as a backstop"; it was not a backstop, and it now says what the code does.
